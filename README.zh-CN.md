...
- 底层模型路由也遵守 `router.mode: "off"`，防止直接调用或关闭前已排队的决策继续切换模型；增加跨 provider 候选认证/切换失败、只向上回退及思考等级映射/不支持的回归。
...
- `retryJudge.appendToResult: false`（默认）只在本地计数失败，不自动请求 Jev，也不改工具输出。显式启用注记后才请求 Jev；后端不可用仍计数。自动持久化失败记忆还需开启注记并设置 `memoryGate.mode: "auto"`。
...
路由需要全局及 `router.enabled` 同时开启。`router.mode: "off"` 禁用所有路由入口；默认 `rules-only` 只响应模型显式请求或用户等级覆盖，不自动推断切换。`advisory` 的自动判断只记录建议，但显式请求仍可切换；`tier-only` 始终不切换；`set-model` 恢复旧式自动路由及判断不可用时的确定性上调回退。
